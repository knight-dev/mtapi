...
namespace MirrorTrader.Fix44
{
    public class MessageConstructor
    {
        ...
        public string HeartbeatMessage(SessionQualifier qualifier, int messageSequenceNumber, string TestReqID = "")
        {
            ...
            if (TestReqID != "")
            {
                ...
                var headerAndBody = header + body;
                trailer = ConstructTrailer(headerAndBody);
                headerAndMessageAndTrailer = header + body + trailer;
            }
            return headerAndMessageAndTrailer.Replace("|", "\u0001");
        }
        ...
        /// <returns></returns>
        public string NewOrderSingleMessage(SessionQualifier qualifier, int messageSequenceNumber, string orderID, long symbol, int side, string transactTime, double orderQuantity, int orderType, string timeInForce, decimal price = 0, decimal stopPrice = 0, string expireTime = "", string positionID = "")
        {
            var body = new StringBuilder();
        ...
        private int CalculateChecksum(string dataToCalculate)
        {
            ...
            return checksum % 256;
            //return checksum(dataToCalculate);
        }
